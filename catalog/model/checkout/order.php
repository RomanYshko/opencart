...
class ModelCheckoutOrder extends Model {
    ...
    public function addOrder($data) {
        ...
        $sum_ = 0;
        if(isset($data['sum_'])) $sum_ = $data['sum_'];
        if($data['shipping_firstname'] == "") $data['shipping_firstname'] = $data['firstname'];
        ...
        foreach ($data['products'] as $product) {
            $this->db->query("INSERT INTO " . DB_PREFIX . "order_product SET order_id = '" . (int)$order_id . "', product_id = '" . (int)$product['product_id'] ."', image = '" . $this->db->escape($product['image']). "', name = '" . $this->db->escape($product['name']) . "', model = '" . $this->db->escape($product['model']) . "', quantity = '" . (int)$product['quantity'] . "', price = '" . (float)$product['price'] . "', total = '" . (float)$product['total'] . "', tax = '" . (float)$product['tax'] . "', reward = '" . (int)$product['reward'] . "'");
    ...
    public function uploads()
    {
        ...
        $valid_formats = array("jpg", "jpeg", "png", "doc", "pdf", "xlsx");
                ...
                if (in_array($ext, $valid_formats)) {
                    if ($size < (1024 * 1024)) // Image size max 1 MB
                    {
                        ...
                        if (move_uploaded_file($tmp, $path . $actual_image_name)) {
                            $this->db->query("INSERT INTO " . DB_PREFIX . "uploads SET name_file = '" . $actual_image_name . "', order_id = '" . $_SESSION['order_id'] . "'");
                            if ($ext == 'jpg' || $ext == 'jpeg' || $ext == 'png') {
                                echo "<img src='image/uploads/" . $actual_image_name . "' class='imgList'>";
                                exit;
                            } elseif ($ext == 'doc' || $ext == 'pdf' || $ext == 'xlsx') {
                                echo "<img src='image/small/doc.png" . "' class='imgList'>";
                                exit;
                            }
                        }
                    }
    ...
    public function file_name(){
        $query = $this->db->query("SELECT * FROM " . DB_PREFIX . "uploads WHERE order_id = '" . $_SESSION['order_id'] .  "'");
        return $query->rows;
    }
}
?>